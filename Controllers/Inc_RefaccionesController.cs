...
public class Inc_RefaccionesController : Controller
{
    private readonly AppDbContext _context;
    private readonly IWebHostEnvironment _env;

    public Inc_RefaccionesController(AppDbContext context, IWebHostEnvironment env)
    {
        _context = context;
        _env = env;
    }
    ...
    [HttpGet]
    public IActionResult solicitud_incadea_refacciones()
    {
        ViewBag.Usuario = HttpContext.Session.GetString("Usuario");
        if (string.IsNullOrEmpty(ViewBag.Usuario)) return RedirectToAction("Index", "Home");
        
        return View("~/Views/Home/solicitud_incadea_refacciones.cshtml");
    }

    [HttpPost]
    public async Task<IActionResult> GuardarIncRefacciones(Incadea incadea, List<IFormFile> evidenciaFiles)
    {
        ...
        )
        {
            ...
            return View("~/Views/Home/solicitud_incadea_refacciones.cshtml", incadea);
        }
        ...
        try
        {
            ...
            incadea.area = "Refacciones";
            _context.Incadea.Add(incadea);
            await _context.SaveChangesAsync();

            TempData["SuccessMessage"] = "Solicitud de Incadea del área de refacciones ha sido registrada correctamente.";
            return RedirectToAction("solicitud_incadea_refacciones", "Inc_Refacciones");
        }
        catch (Exception ex)
        {
